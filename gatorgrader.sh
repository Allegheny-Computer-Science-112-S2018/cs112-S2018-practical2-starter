...
if [ "$CHECK" = true ]; then
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicaltwo/ \
                                     --checkfiles StatisticsCalculator.java --multicomments 5 --language Java
  determine_exit_code $?
  # --> GatorGrader CHECK: at least k "println()" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicaltwo \
                                     --checkfiles StatisticsCalculator.java --fragments "println(" --fragmentcounts 5
  determine_exit_code $?
  # --> GatorGrader CHECK: at least k "println()" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/test/java/practicaltwo \
                                     --checkfiles TestStatisticsCalculator.java --fragments "@Test" --fragmentcounts 9
  determine_exit_code $?
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --fragments "6.78"
  determine_exit_code $?
  # --> GatorGrader CHECK: running the program produces the required fragment
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --fragments "23.04"
  determine_exit_code $?
  # --> GatorGrader CHECK: running the program produces the required fragment
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --fragments "4.80"
  determine_exit_code $?
